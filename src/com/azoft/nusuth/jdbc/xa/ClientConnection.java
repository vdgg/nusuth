...
final class ClientConnection
    ...
    private static final Category CAT = Category.getInstance(ClientConnection.class.getName());
     ...
     */
    ClientConnection(XAConnectionImpl xaConn, int clientId) throws SQLException {
        if (xaConn == null) {
            CAT.error("Can not create connection proxy by null XAConnection");
            throw new SQLException("Can not create connection proxy by null XAConnection");
        }

        _xaConn = xaConn;
        _clientId = clientId;

        //check connection for validness.
        Connection con = _xaConn.getUnderlying(_clientId);
        if (con == null) {
    ...
    public void close() throws SQLException {
        XAConnectionImpl xaConn = _xaConn;
        if (xaConn == null)
            return;
        ...
        // a concern of us.
        if (CAT.isDebugEnabled()) CAT.debug("Closing ClientConnection");

        xaConn.notifyClose(_clientId);
        synchronized (this) {
            _xaConn = null;
        }
    }
    ...
    */
    ...
    public String toString() {
        try {
            return "ClientConnection, underlying = " + getUnderlying().toString();
        } catch (SQLException except) {
